/***************************************************************************************
    Name    : Respirator - Arduino - LCD Button Shield Menu - Open Source
    Author  : Paul Siewert (modified by Luke Lozowski)
    ...
    Notes   : This code is for use with an Arduino Uno and LCD/button shield. The
              ...
              required by the user. The parts of the code been add to enable pins
              2 and 3 to send a signal to the motor/pump and to the valve with the
              oxygen supply.
...
/****************************************************************************************/
/*
   Respirator code been add to enable to send signal to PIN 2 and PIN 3 based on the settings from the menu. On default it is set to OFF and the trun in ON,
   you need to scroll down to option 3 in a menu. Currently it is set on default to be on for 2 seconds and off for 3 seconds. This is not correct time for the respirator!!!
 */

String menuItems[] = {"OnTime", "OffTime", "ON/OFF"};
...
// Respirator menu variables
 int onTime = 2000;
 int offTime = 3000;
 int onoff = 0;

// Respirator hardware parts
...
// This function will generate the signal to pin 2 and 3 with a delay set in a menu.
void respiratorProces(){
  if (onoff == 1) {
    // turn Respirator Proces on:
  ...
  while (activeButton == 0) {
    ...
    switch (button) {
      case 2:
        button = 0;
        onTime = onTime + 100;
        onTime = constrain(onTime,0,5000);
        lcd.setCursor(10,0);
        lcd.print("     ");
        lcd.setCursor(10,0);        
        lcd.print(onTime);
  ...
  while (activeButton == 0) {
    ...
    switch (button) {
      case 2:
        button = 0;
        offTime = offTime + 100;
        offTime = constrain(offTime,0,5000);
        lcd.setCursor(10,0);
        lcd.print("     ");
        lcd.setCursor(10,0);        
        lcd.print(offTime);
